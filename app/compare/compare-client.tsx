...
export function CompareClient({ roles, getRoleSkills, isDemoMode }: Props) {
  ...
  const loadSkillsForComparison = async () => {
    ...
    setIsLoading(true)
    try {
      const [skills1, skills2] = await Promise.all([getRoleSkills(selectedRole1), getRoleSkills(selectedRole2)])
